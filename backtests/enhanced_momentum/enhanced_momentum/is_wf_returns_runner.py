# прогоняет ens-3 на walk-forward IS test years 2015–2019 и сохраняет daily returns
...
ENS3_CONFIGS: list[dict[str, Any]] = [
    {
...
def _worker_run_one(params: dict[str, Any], results_subdir: str) -> None:
    ...
    if (
        metrics_path.exists()
        and (out_dir / "strategy_total_r.parquet").exists()
        and (out_dir / "strategy_excess_r.parquet").exists()
    ):
        return
...
def main() -> None:
    ...
    parser.add_argument("--results-subdir", default="data/results_is_wf_with_returns")
    parser.add_argument("--windows", default="wf_2015,wf_2016,wf_2017,wf_2018,wf_2019")
    ...
    windows = [w for w in WF_WINDOWS if w["eval_window"] in selected_windows]
    ...
    for window in windows:
        for cfg in ENS3_CONFIGS:
            jobs.append(
                {
                    "strategy": "SystematicMomentum",
                    "experiment_group": "is_wf_ens3_returns",
    ...
    total = len(jobs)
    print(f"Total IS WF jobs: {total}")
    ...
    for i, params in enumerate(jobs, start=1):
        ...
        if metrics_path.exists() and total_path.exists() and excess_path.exists():
            print(f"{msg} [cache]")
            continue
    ...
    print("IS WF returns runner finished.")
